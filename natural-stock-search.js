...
function extractNaturalStockKeyword(
  ...
  return String(
    ...
    .replace(
      /(情報をまとめて|まとめて教えて|まとめて|まとめ|全部教えて|全部|情報|状況|について|次の船便|次便|船便|船積み数量|船積数量|船積み|船積|船(?=\s|$)|積載数量|積載|振り分け数量|振分数量|振り分け|振分|載っている|載ってる|載る|積んでいる|積んでる|積む|販売予定|出荷予定|出荷時期|出荷日|出荷|販売|予定|現在庫数|現在庫|合計在庫数|合計在庫|在庫数|場所別在庫|在庫|保管場所|どこに|どこ|いつ|何個|何本|何箱|何枚|何台|何袋|何セット|いくつ|ありますか|あるの|ある|教えてください|教えて|知りたい|確認したい|検索して|検索|商品コード|社内コード|JANコード|JAN)/gi,
      " "
    )
    .replace(
      /[はがをのにでと？?！!。、「」『』]/g,
...
function extractNaturalShippingScheduleKeyword(
  ...
  return normalizeNaturalStockText(
    ...
    .replace(
      /(船便|船積み|船積|船(?=\s|$)|積載|振り分け|振分|について|の情報|情報|詳細|教えてください|教えて|確認したい|確認|見たい|検索して|検索|どんな|どの|いつ|次の|次便)/gi,
      " "
    )
    .replace(
      /[はがをのにでと？?！!。、「」『』]/g,
